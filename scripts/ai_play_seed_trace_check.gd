...
const TRACE_HAND_OFFSETS := [2]
const TRACE_DIFFICULTIES := [2]
...
func run() -> void:
	...
	for difficulty in TRACE_DIFFICULTIES:
		for hand_offset in TRACE_HAND_OFFSETS:
			...
			print("MATCH diff=%d hand=%d seed=%d deal_in=%d winner=%d terminal=%s step=%d high=%d/%d human_high=%d/%d" % [
				difficulty,
				hand_offset,
				...
				int(result.get("terminal_trace_step", -1)),
			...
			for entry in result.get("discard_trace", []):
				if int(entry.get("seat", -1)) != 0 and float(entry.get("risk", 0.0)) < scene.AI_DANGER_RISK_HIGH:
					continue
				print("TRACE step=%d seat=%d tile=%s rank=%d risk=%.1f feed=%.1f human=%.1f sh=%d score=%.1f best=%s/%.1f/sh%d safe=%s/%.1f/feed%.1f/h%.1f/sh%d/score%.1f wait=%d/%d value=%.1f catastrophe=%s moved=%s guard_from=%s gap=%.1f gain=%.1f" % [
					...
					float(entry.get("safest_feed", 0.0)),
					float(entry.get("safest_human_pressure", 0.0)),
					int(entry.get("safest_shanten", -1)),
					float(entry.get("safest_score", 0.0)),
					int(entry.get("wait_best_points", 0)),
					int(entry.get("wait_total_remaining", 0)),
					float(entry.get("wait_value", 0.0)),
					str(entry.get("hard_guard_catastrophe_tenpai", false)),
					str(entry.get("hard_guard_moved", false)),
					str(entry.get("hard_guard_from_tile", "")),
					float(entry.get("hard_guard_safe_score_gap", 0.0)),
					float(entry.get("hard_guard_safe_pressure_gain", 0.0)),
				])
