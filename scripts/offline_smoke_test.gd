...
func run() -> void:
	...
	check(count_named_nodes(scene, "ShopItemRowLowFrequencyPlate") == scene.ITEM_TYPES.size(), "shop item rows expose one low-frequency authored reading plate per item")
	...
	scene.start_offline(false)
	scene.game_stats = {"games_played": 12, "games_won": 7, "total_score": 18800, "best_score": 9600, "win_rate": 7.0 / 12.0, "total_fan": 36, "total_hands": 44}
	...
	check(stats_winrate_value != null and stats_winrate_value.text == "顺风" and stats_winrate_unit != null and stats_winrate_unit.text == "状态" and stats_games_value != null and stats_games_value.text == "3" and stats_games_unit != null and stats_games_unit.text == "番/局" and stats_best_value != null and stats_best_value.text == scene.compact_score_text(1567) and stats_best_unit != null and stats_best_unit.text == "分/局", "stats dashboard keeps trend and per-round summary value/unit labels structurally separate")
	...
	check(scene.find_child("RulesReadingStatusPlate", true, false) != null and scene.find_child("RulesReadingStatus", true, false) != null, "rules screen exposes the current reading position beside the guide")
	...
	check(scene.bounded_online_input("  ABCDE  ", 3) == "ABC" and scene.online_connection_endpoint_text() == "%s:%d" % [scene.DEFAULT_HOST, scene.DEFAULT_PORT], "online lobby exposes bounded input and actual-endpoint contracts")
	check(scene.online_name_edit.max_length == scene.ONLINE_NAME_MAX_LENGTH and scene.online_host_edit.max_length == scene.ONLINE_HOST_MAX_LENGTH and scene.online_room_edit.max_length == scene.ONLINE_ROOM_CODE_MAX_LENGTH, "online lobby applies explicit client length boundaries")
	...
	var online_log_scroll = scene.find_child("OnlineLobbyLogScroll", true, false) as ScrollContainer
	var online_log_text = scene.find_child("OnlineLobbyLogListText", true, false) as RichTextLabel
	check(scene.find_child("OnlineLobbyLogListPanel", true, false) != null and scene.find_child("OnlineLobbyLogCountBadge", true, false) != null and scene.find_child("OnlineLobbyLogCountLabel", true, false) != null and online_log_scroll != null and online_log_text != null, "online lobby renders a dedicated native-scroll room log list")
	check(online_log_scroll != null and online_log_text != null and online_log_text.get_parent() == online_log_scroll and online_log_text.fit_content and not online_log_text.scroll_active, "online lobby delegates fourteen-entry log overflow to the native scroll container")
