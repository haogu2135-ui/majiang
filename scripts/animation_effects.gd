	...
	if new_texture != null:
		...
		if sprite != null:
			...
			tw.chain().tween_callback(func():
				var sprite_node := instance_from_id(sprite_id) as Sprite2D
				if sprite_node != null:
...
static func animate_number(label: Label, target_value: int, duration: float = DURATION_SLOW) -> Tween:
	if label == null or not is_instance_valid(label) or not label.is_inside_tree():
		return null
	...
	for i in range(steps):
		...
		tw.tween_callback(func():
			var label_node := instance_from_id(label_id) as Label
			if label_node != null:
				label_node.text = str(next_value)
