...
REVISION="$(git -C "$ROOT_DIR" rev-parse --short HEAD 2>/dev/null || printf 'unknown')"
{
	...
	echo "- Contract: 10 independent seeds, 2 paired easy/hard hands per seed, fixed normal player probe, per-seed terminal/tile/score integrity, and aggregate difficulty/actionable-pressure gates"
	echo "- Log: $LOG"
