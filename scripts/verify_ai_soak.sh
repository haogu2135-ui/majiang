...
REVISION="$(git -C "$ROOT_DIR" rev-parse --short HEAD 2>/dev/null || printf 'unknown')"
{
	...
	echo "- Contract: 14 independent seeds, 2 paired easy/hard hands per seed, fixed normal player probe, per-seed terminal/tile/score integrity, and aggregate gates (8 pp actionable-pressure and 8 pp player deal-in tolerances)"
	echo "- Log: $LOG"
