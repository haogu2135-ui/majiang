...
| P13 | 新手引导：用教程解释摸切、吃碰杠胡、响应窗口和牌势提示，降低首次流失。 | 当前已有教程状态和页面提示，但缺少完整新用户漏斗、跳过策略和教学完成率。 | `scripts/main_base.gd`、`scripts/main_src/screens.gd.part`、`scripts/main_src/render.gd.part` | 首次进入可完成一条不依赖真人的教学流程；中断后可继续；完成状态持久化且不打扰老用户。 | 部分完成 |
