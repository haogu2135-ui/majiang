...
下方 2026-06-15 APK 静态校验为历史记录。当前构建已推进到 `1.0.165-godot`：`project.godot`、`scripts/main_base.gd`、`export_presets.cfg` 和四个本地 APK 别名一致；`aapt dump badging` 可读到 `versionCode='165' versionName='1.0.165-godot'`；四个 APK size 均为 `153949761`，SHA-256 均为 `10fa1f1a4ffeefad6984183729549cd58bbbd14245044d88e20994ecdf26fcac`；`apksigner verify --verbose` 均通过 v2/v3；版本化包和 latest APK 内未发现旧的 `assets/build/YunzhuoMahjongGodot-update.json`。当前未发现需要重新生图的 APP UI 资产缺口，剩余 APP 级风险仍是真机/ADB 安装、音频听感、触控与安全区验收。
