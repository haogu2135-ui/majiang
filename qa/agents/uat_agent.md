...
下方 2026-06-15 记录为历史只读 UAT。当前构建已经推进到 `1.0.165-godot`，原先的 `1.0.105`/`1.0.103` 版本错配不再代表当前状态。

- 生图需求先行复核：当前 95 个已登记 GPT 插图 PNG 均存在，候选/QA `.import` 噪音为 0，最新截图未发现缺图、假透明棋盘格、坏 alpha、占位图或需要重新生成的图片资产；未派发生图任务。
- 当前版本源一致：`project.godot`、`scripts/main_base.gd`、`export_presets.cfg` 均为 `1.0.165-godot` / `versionCode=165`。
- 当前 APK 一致：`build/YunzhuoMahjongGodot-v1.0.165-godot.apk`、`build/YunzhuoMahjongGodot-release.apk`、`build/YunzhuoMahjongGodot.apk`、`build/YunzhuoMahjong.apk` 大小均为 `153949761`，SHA-256 均为 `10fa1f1a4ffeefad6984183729549cd58bbbd14245044d88e20994ecdf26fcac`。
- 当前 update manifest 一致：`build/YunzhuoMahjongGodot-update.json` full package 的 size/SHA 与上述 APK 匹配，URL 指向 `YunzhuoMahjongGodot-v1.0.165-godot.apk`。
- 静态 APK 复核：`aapt dump badging` 可读到 `versionCode='165' versionName='1.0.165-godot'`；`apksigner verify --verbose` 对四个 APK 均通过 v2/v3；版本化包和 latest APK 内未发现旧 `assets/build/YunzhuoMahjongGodot-update.json`。
- 仍需真机 UAT：音频听感/卡顿、触控延迟、刘海/手势安全区、真实安装和在线更新流程尚未由 ADB 或 Android 设备验证。
