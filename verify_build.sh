...
echo ""
echo "3b. 检查 main.gd 模块化结构..."
ASSEMBLE_TOOL="$SCRIPT_DIR/tools/assemble_main.py"
if [ -f "$ASSEMBLE_TOOL" ] && command -v python3 >/dev/null 2>&1; then
    # part 合法性与函数唯一性
    if python3 "$ASSEMBLE_TOOL" --check >/dev/null 2>&1; then
        check_pass "main_src/*.gd.part 校验通过（无重复 func、无非法顶层关键字）"
    else
        check_fail "main_src/*.gd.part 校验" "存在重复 func 或非法顶层关键字"
    fi
    # main.gd 是否与生成的产物一致（防止有人手改产物却未跑拼接器）
    if [ -f "$MAIN_SCRIPT" ]; then
        if python3 "$ASSEMBLE_TOOL" --verify >/dev/null 2>&1; then
            check_pass "main.gd 与 assemble_main 生成产物一致"
        else
            check_fail "main.gd 与生成产物一致性" "请运行 python tools/assemble_main.py 重新生成"
        fi
    fi
else
    if [ ! -f "$ASSEMBLE_TOOL" ]; then
        check_fail "tools/assemble_main.py" "构建工具缺失"
    else
        check_fail "python3 环境" "未找到 python3"
    fi
fi
...
if command -v godot >/dev/null 2>&1; then
    if GODOT_SILENCE_ROOT_WARNING=1 godot --headless --path "$SCRIPT_DIR" -s scripts/ui_layout_smoke_test.gd >/tmp/yunzhuo_ui_layout_smoke.log 2>&1; then
        check_pass "UI布局冒烟测试通过"
    else
        check_fail "UI布局冒烟测试" "详见 /tmp/yunzhuo_ui_layout_smoke.log"
    fi
else
    check_fail "godot 环境" "未找到 godot，无法运行UI布局冒烟测试"
fi
